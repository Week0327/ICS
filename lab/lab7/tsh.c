...
int builtin_cmd(char **argv)
{
	...
	if (!strcmp(argv[0], "quit")) //若第一个参数为quit
	{
	...
	else if (!strcmp(argv[0], "fg") || !strcmp(argv[0], "bg")) //若第一个参数为fg或bg
	{
		do_bgfg(argv);
		return 1; //返回：“是” 内部参数
	}
...
void sigchld_handler(int sig)
{
	...
	{
		if (WIFEXITED(status)) //若子进程是通过exit或return正常退出
			deletejob(jobs, pid);
		...
		else if (WIFSTOPPED(status)) //若子进程是收到信号而挂起
		{
			...
			getjobpid(jobs, pid)->state = ST;			 //将此进程标记为挂起的进程
		}
	}
...
void sigtstp_handler(int sig)
{
	...
	for (int i = 0; i < MAXJOBS; i++)
	{
		if (jobs[i].pid != 0)
		{
			if (jobs[i].state == FG)
			{
				if (kill(-(jobs[i].pid), SIGTSTP) < 0) //找到前台进程，并发送信号将其停止
					unix_error("kill error\n");
			}
		}
	}
